...
{
    public class OvniToyMono_MoveFromControllerPosition : MonoBehaviour
    {

        public Transform m_controllerToObserve;
        ...
        private void Reset()
        {
            m_controllerToObserve = this.transform;
        }
        public void Update()
        {

          
            if (m_controllerToObserve == null)
                return;

            Vector3 directionForward = m_controllerToObserve.forward;
            ...
            if (m_useDebugLine)
            {
                Debug.DrawLine(m_controllerToObserve.position, m_ovniToMove.position, Color.yellow);
